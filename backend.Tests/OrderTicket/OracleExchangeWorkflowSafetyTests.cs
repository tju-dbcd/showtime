        ...
        public static async Task<OracleWorkflowFixture> CreateAsync(decimal targetPrice)
        {
            ...
            try
            {
                ...
                db.AddRange(
                    ...
                    {
                        SessionId = OriginalSessionId, ShowId = showId, SeatMapId = seatMapId,
                        StartTime = now.AddDays(7), EndTime = now.AddDays(7).AddHours(2),
                        SaleStartTime = now.AddDays(-1), SaleEndTime = now.AddDays(6),
                        SessionStatus = "ONSALE", CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new ShowtimeBackend.Entities.ShowSession.ShowSession
                    {
                        SessionId = TargetSessionId, ShowId = showId, SeatMapId = seatMapId,
                        StartTime = now.AddDays(8), EndTime = now.AddDays(8).AddHours(2),
                        SaleStartTime = now.AddDays(-1), SaleEndTime = now.AddDays(7),
                        SessionStatus = "ONSALE", CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new SeatSection
                    {
                        SeatSectionId = SeatSectionId, SeatMapId = seatMapId,
                        SectionCode = "EXGATE", SectionName = "Exchange gate",
                        CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new Seat
                    {
                        SeatId = OriginalSeatId, SeatSectionId = SeatSectionId,
                        RowCode = "A", SeatNo = "1", RowIndex = 1, ColIndex = 1,
                        CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new Seat
                    {
                        SeatId = TargetSeatId, SeatSectionId = SeatSectionId,
                        RowCode = "A", SeatNo = "2", RowIndex = 1, ColIndex = 2,
                        CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new PriceStrategy
                    {
                        PriceStrategyId = OriginalStrategyId, SessionId = OriginalSessionId,
                        SeatSectionId = SeatSectionId, StrategyName = "original gate",
                        Price = 105m, SaleStartTime = now.AddDays(-1), SaleEndTime = now.AddDays(6),
                        CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new PriceStrategy
                    {
                        PriceStrategyId = TargetStrategyId, SessionId = TargetSessionId,
                        SeatSectionId = SeatSectionId, StrategyName = "target gate",
                        Price = targetPrice, SaleStartTime = now.AddDays(-1), SaleEndTime = now.AddDays(7),
                        CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new Order
                    {
                        OrderId = OriginalOrderId, OrderNo = "EXGATE-ORDER-998820001",
                        UserId = userId, SessionId = OriginalSessionId,
                        TotalAmount = 105m, TicketCount = 1, OrderStatus = "ISSUED",
                        ExpireTime = now.AddDays(-1), PayTime = now.AddDays(-1),
                        IssueTime = now.AddDays(-1), Source = "WEB",
                        CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new OrderItem
                    {
                        OrderItemId = OriginalItemId, OrderId = OriginalOrderId,
                        SeatId = OriginalSeatId, PriceStrategyId = OriginalStrategyId,
                        UnitPrice = 105m, ItemStatus = "NORMAL",
                        CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new Payment
                    {
                        PaymentId = OriginalPaymentId, PaymentNo = "EXGATE-PAY-998820001",
                        OrderId = OriginalOrderId, UserId = userId, PayAmount = 105m,
                        PayChannel = "ALIPAY", PayStatus = "SUCCESS", PayTime = now.AddDays(-1),
                        CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new ETicket
                    {
                        ETicketId = OriginalTicketId, ETicketNo = "EXGATE-TKT-998820001",
                        OrderItemId = OriginalItemId, UserId = userId,
                        QrCode = "exgate-original-qr", AntiFakeCode = "exgate-original-anti",
                        TicketStatus = "UNUSED", CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new SeatReservation
                    {
                        SeatReservationId = OriginalReservationId,
                        SessionId = OriginalSessionId, SeatId = OriginalSeatId,
                        OrderItemId = OriginalItemId, ReservationType = "ORDER",
                        ReservationStatus = "ACTIVE", ReserveTime = now.AddDays(-1),
                        CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new SeatLock
                    {
                        SeatLockId = TargetLockId, SessionId = TargetSessionId,
                        SeatId = TargetSeatId, UserId = userId,
                        LockToken = "oracle-exchange-lock", LockStatus = "ACTIVE",
                        LockTime = now, ExpireTime = now.AddHours(1),
                        CreateTime = now, CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new ExchangePolicy
                    {
                        PolicyId = WorkflowPolicyId, ShowId = showId,
                        PolicyName = "oracle workflow gate", ExchangeDeadlineHour = 24,
                        ExchangeFee = 5m, AllowCrossSession = 1, Priority = 100, Status = 1,
                        CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    },
                    new RefundPolicy
                    {
                        PolicyId = WorkflowRefundPolicyId, ShowId = showId,
                        PolicyName = "oracle refund race gate", RefundDeadlineHour = 0,
                        RefundRate = 1m, ServiceFee = 0m, Priority = 100, Status = 1,
                        CreateBy = "oracle-gate", UpdateBy = "oracle-gate",
                    });
